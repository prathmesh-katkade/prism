...
const captureSet = process.env.ATLAS_CAPTURE_SET ?? 'before';
const captureDir = `docs/atlas/vnext/${captureSet}`;
...
(async () => {
  ...
  await page.screenshot({ path: `${captureDir}/home.png`, fullPage: true });
  await page.getByRole('button', { name: /Atlas native/i }).click();
  await page.waitForTimeout(2000);
  await page.screenshot({ path: `${captureDir}/workspace-empty.png`, fullPage: true });
  console.log((await page.locator('body').innerText()).slice(0, 7000));
  await page.getByRole('button', { name: 'System status' }).click();
  await page.waitForTimeout(1500);
  await page.screenshot({ path: `${captureDir}/command-center.png`, fullPage: true });
  ...
  await page.getByRole('button', { name: /Atlas native/i }).click();
  await page.screenshot({ path: `${captureDir}/workspace.png`, fullPage: true });
  ...
  await page.screenshot({ path: `${captureDir}/cortex-core.png`, fullPage: true });
  ...
  await page.screenshot({ path: `${captureDir}/cortex-node.png`, fullPage: true });
  await page.getByLabel('Atlas request pipeline').getByRole('button', { name: /Result/ }).click();
  await page.screenshot({ path: `${captureDir}/cortex-pipeline.png`, fullPage: true });
  const group = page.getByLabel('Cortex satellites').getByRole('button', { name: 'Focus Active dataset revision' });
  await group.click();
  await page.screenshot({ path: `${captureDir}/cortex-group.png`, fullPage: true });
  await page.getByRole('tab', { name: 'Summary' }).click();
  await page.screenshot({ path: `${captureDir}/inspector.png`, fullPage: true });
  await browser.close();
