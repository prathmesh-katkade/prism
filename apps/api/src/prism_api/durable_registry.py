...
_audit_events = Table(
    "prism_analytical_audit_events", _metadata,
    Column("event_id", String(36), primary_key=True),
    Column("object_id", String(255), nullable=False, index=True),
    Column("event_type", String(64), nullable=False),
    Column("actor", String(255), nullable=False),
    Column("producer_service", String(255), nullable=False),
    Column("producer_version", String(255), nullable=False),
    Column("request_id", String(255), nullable=True, index=True),
    Column("created_at", DateTime(timezone=True), nullable=False, index=True),
)
...
def history_database_url() -> str:
    ...
    if configured:
        return configured
    if os.environ.get("PRISM_REQUIRE_DURABLE_HISTORY", "").lower() in {"1", "true", "yes"}:
        raise RuntimeError(
            "PRISM_ANALYTICAL_HISTORY_DATABASE_URL is required when durable analytical history is enabled."
        )
...
class DurableAnalyticalObjectRegistry(AnalyticalObjectRegistry):
    ...
    def __init__(self, database_url: Optional[str] = None) -> None:
        # Existing Phase 8 unit tests construct ``registry.__class__()`` to get
        # an isolated, deliberately-corruptible graph.  Keep that useful test
        # seam in memory; the application always enters through the factory
        # below with an explicit durable URL.
        self._in_memory = database_url is None
        super().__init__()
        self._audit_memory: list[dict[str, str | None]] = []
        if self._in_memory:
    ...
    @staticmethod
    def _restore_snapshot(snapshot: str) -> AnalyticalObject:
    ...
    def register(self, record: AnalyticalObject) -> AnalyticalObject:
        if self._in_memory:
            registered = super().register(record)
            event = self._audit_value(registered)
            self._audit_memory.append({**event, "created_at": registered.provenance.created_at.isoformat()})
            return registered
    ...
    def _audit_value(record: AnalyticalObject) -> dict[str, Any]:
        producer = record.provenance.reproducibility.producer
        return {
            "event_id": str(uuid.uuid4()),
            "object_id": record.object_id,
            "event_type": "created",
            # PRISM has no authenticated principal at this architecture layer;
            # label that fact rather than inventing an actor identity.
            "actor": "system",
    ...
    def list_audit_events(self, object_id: str, limit: int = 100) -> list[dict[str, str | None]]:
        """Return append-only creation events; no lineage/audit mutation route exists."""
        if self._in_memory:
            return [event.copy() for event in self._audit_memory if event["object_id"] == object_id][:limit]
        statement = (
            select(_audit_events)
            .where(_audit_events.c.object_id == object_id)
            .order_by(desc(_audit_events.c.created_at), desc(_audit_events.c.event_id))
            .limit(limit)
        )
        assert self.engine is not None
        with self.engine.connect() as connection:
        ...
        return [
            {
                ...
                "actor": str(row["actor"]),
                "producer_service": str(row["producer_service"]),
    ...
    def get(self, object_id: str) -> Optional[AnalyticalObject]:
        ...
        return None if snapshot is None else self._restore_snapshot(snapshot)

    def exists(self, object_id: str) -> bool:
        if self._in_memory:
            return super().exists(object_id)
    ...
    def list_for_dataset(self, dataset_id: str, revision: Optional[int] = None, kind: Optional[ObjectKind] = None) -> list[AnalyticalObject]:
        if self._in_memory:
            return super().list_for_dataset(dataset_id, revision=revision, kind=kind)
        ...
        assert self.engine is not None
        with self.engine.connect() as connection:
            return [self._restore_snapshot(snapshot) for snapshot in connection.execute(statement).scalars()]
    ...
    def list_recent(self, limit: int = 100, kind: Optional[ObjectKind] = None) -> list[AnalyticalObject]:
        ...
        if self._in_memory:
            ...
            records.sort(key=lambda item: (item["provenance"]["created_at"], item["object_id"]), reverse=True)
            restored = [AnalyticalObjectRegistry._restore(item) for item in records]
            return [item for item in restored if kind is None or item.kind is kind][:limit]
    ...
    def _neighbor_ids(self, object_id: str, direction: str) -> list[str]:
        if self._in_memory:
            return super()._neighbor_ids(object_id, direction)
    ...
    def _fetch_records(self, object_ids: Iterable[str]) -> list[AnalyticalObject]:
        ...
        by_id = {object_id: self._restore_snapshot(snapshot) for object_id, snapshot in rows}
    ...
    def get_parents(self, object_id: str) -> Optional[list[AnalyticalObject]]:
        if self._in_memory:
            return super().get_parents(object_id)
        if not self.exists(object_id):
            return None
        return self._fetch_records(self._neighbor_ids(object_id, "ancestors"))

    def get_children(self, object_id: str) -> Optional[list[AnalyticalObject]]:
        if self._in_memory:
            return super().get_children(object_id)
        if not self.exists(object_id):
            return None
        return self._fetch_records(self._neighbor_ids(object_id, "descendants"))

    def _traverse(self, object_id: str, direction: str, max_depth: Optional[int]) -> Optional[TraversalResult]:
        if self._in_memory:
            return super()._traverse(object_id, direction, max_depth)
        if not self.exists(object_id):
            return None
        ...
        while frontier:
            ...
            depth += 1
        records = {record.object_id: record for record in self._fetch_records(depth_by_id)}
    ...
    def ancestors(self, object_id: str, max_depth: Optional[int] = None) -> Optional[TraversalResult]:
        if self._in_memory:
            return super().ancestors(object_id, max_depth=max_depth)
    ...
    def descendants(self, object_id: str, max_depth: Optional[int] = None) -> Optional[TraversalResult]:
        if self._in_memory:
            return super().descendants(object_id, max_depth=max_depth)
    ...
    def shortest_path(self, from_object_id: str, to_object_id: str) -> Optional[PathResult]:
        if self._in_memory:
            return super().shortest_path(from_object_id, to_object_id)
            ...
            for current in frontier:
                ...
                for neighbor, relation in sorted(parents + children):
                    ...
                    if neighbor == to_object_id:
                        ...
                        chain.reverse()
                        records = {item.object_id: item for item in self._fetch_records(chain)}
...
def create_history_registry() -> DurableAnalyticalObjectRegistry:
    return DurableAnalyticalObjectRegistry(history_database_url())
