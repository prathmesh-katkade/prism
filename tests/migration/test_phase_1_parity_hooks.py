...
def test_phase_6_promotes_clean_and_visualize_after_their_own_parity_evidence() -> None:
    ...
    e2e gate passed.
    """
    assert {state.workflow for state in PHASE_1_MIGRATIONS} == {"overview", "sql-lab", "ai-analyst", "clean", "visualize", "stats", "forecasting", "ml"}
    ...
    assert channels["ml"] is ReleaseChannel.ENABLED
