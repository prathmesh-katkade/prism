...
## Round 5 re-adjudication — 2026-09-25

[The read-only rescore](round-5-rescored.json) applies [ADR 0017](../../architecture/adr/0017-atlasbench-critical-item-allowance.md) to the same ten immutable Round 5 suite rows. A critical category now rejects on two or more missing items, while any lower overall score still rejects. No benchmark response was regenerated. The stored evaluation policy ID remains historical; new runs use the versioned policy ID that includes the one-item allowance.

Production remains 90/90. `granite4:micro`, `qwen3:8b`, `qwen2.5:7b-instruct`, and `phi4-mini:latest` tie it at 90/90 and are AtlasBench `promote_eligible` under the new rule; this does not certify or promote them. `ministral-3:8b` is **89/90**, losing one statistics item. Its critical-category loss is allowed, but its lower overall score still makes its verdict `reject`. `qwen2.5:3b`, `ministral-3:3b`, and `granite4:tiny-h` also remain below production overall. `olmo-3:7b-instruct` loses two SQL items and two overall, so it remains rejected on both grounds. The prediction that the allowance would admit Ministral 8B contradicts the stored Round 5 scores.